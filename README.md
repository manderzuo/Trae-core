...
- 视频实际扣费控制默认为暂停；只有拿到可核验的任务级上游积分回执后才能安全结算。不要把固定值、token 数或视频时长当作真实扣费。
...
上游无法提供可信的单任务积分上限时，管理员可在“设置 → 视频计费安全状态”为一个普通 Key 登记精确的一次性请求（Key 内部 ID 与请求摘要）。默认仍为 `paused`；只有该 Key 的下一次匹配请求且 AI Work 明确返回 `quote_unavailable`，才进入受控模式。其他 Key、报价超时或鉴权失败都不降级。该路径支持 Seedance Chat（DeepSeek 辅助 + 视频）与原生 `/v1/videos/generations`（仅视频）。

Core 在调用任何付费辅助前，先检查上游积分快照、素材归属和 Key 额度，并将该 Key 全部可用积分持有到同一个操作。这个数值是授权持有额，**不是**上游价格上限；上游真实扣费可能超出它。辅助与视频各自使用独立请求编号查询真实积分回执，只有全部可能收费的步骤已核验才合并实扣一次；超额时完整记账并冻结 Key。未知/冲突回执或断线时保持持有，只查询原请求，不自动重发生成。管理员页显示操作状态、两步回执和待对账情况；未结清时不得为同一 Key 登记下一次。
...
本模式只适合受控验收，不应视为长期无人值守的普遍放行方案。v22→v23 迁移新增操作和步骤账本并保留旧账本；部署前须在停写窗口完整备份数据库及 WAL/SHM，不能只回退二进制。先升级 AI Work 桥接兼容版本，再升级 Core；核对两端健康、受控状态与 Key 持有，最后只发送一次真实请求。回执不明时停止新请求并保留现场，不能凭聚合余额差、固定积分或视频文件单独宣称计费通过。
