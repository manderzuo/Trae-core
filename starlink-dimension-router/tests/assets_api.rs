...
impl BridgeTransport for VideoBridge {
    fn send(&self, method: &str, url: &str, _headers: &BTreeMap<String, String>, body: &[u8]) -> Result<BridgeResponse, String> {
        ...
        let (status, response) = if method == "GET" && path == "/internal/bridge/summary" {
        ...
        } else if method == "POST" && path == "/internal/bridge/quotes" {
            ...
            (200, serde_json::to_vec(&json!({
                ...
                "model":request["model"],
                "max_credits":"0.000010",
                "unit":"credits",
                "expires_at_ms":Utc::now().timestamp_millis()+60_000,
                "source_ref":"asset-api-test-quote"
            })).unwrap())
        } else if method == "GET" && path.starts_with("/internal/bridge/requests/") && path.ends_with("/billing") {
            let request_id = path.trim_start_matches("/internal/bridge/requests/").trim_end_matches("/billing");
            (200, serde_json::to_vec(&json!({
                "request_id":request_id,
                "status":"final",
                "actual_credits":"0.000001",
                "unit":"credits",
                "source_ref":"asset-api-test-receipt",
                "task_ref":"video-1",
                "observed_at_ms":Utc::now().timestamp_millis()
            })).unwrap())
        } else if path == "/v1/assets" {
            (202, br#"{"object":"asset","id":"bridge-asset-1"}"#.to_vec())
        } else if path == "/v1/videos/generations" {
            (202, br#"{"task":{"id":"video-1","status":"queued"},"status":"queued"}"#.to_vec())
        } else if path == "/v1/videos/video-1" {
            (202, br#"{"task":{"id":"video-1","status":"completed","billing":{"status":"verified","actual_credits":"1.000000","unit":"credits","task_ref":"video-1"}}}"#.to_vec())
        } else if path == "/v1/videos/video-1/content" {
            (202, b"\0\0\0\x18ftypmp42test".to_vec())
        } else {
            (202, br#"{}"#.to_vec())
        };
        Ok(BridgeResponse { status, headers: BTreeMap::new(), body: response })
    }
}
...
fn fixture_with_user_scopes(scopes: &[&str]) -> ScopeFixture {
...
fn video_fixture() -> VideoFixture {
    ...
    VideoFixture { app: build_router(state), key: key.plaintext, other_key: other.plaintext, bridge, dir: TestDir(dir) }
}
...
async fn upload_requires_assets_scope_and_invalid_token_is_not_found() {
    let scope_fixture = fixture_with_user_scopes(&["videos:submit"]);
...
#[tokio::test]
async fn nested_upstream_video_id_can_be_polled_and_downloaded_by_its_owner() {
    let fixture = video_fixture();
    let submitted = post_bearer(&fixture.app, "/v1/videos/generations", &fixture.key, json!({"model":"seedance","prompt":"测试视频"})).await;
    assert_eq!(submitted.status(), StatusCode::ACCEPTED);
    assert_eq!(response_json(submitted).await["task"]["id"], "video-1");

    let task = fixture.app.clone().oneshot(Request::get("/v1/videos/video-1").header("authorization", format!("Bearer {}", fixture.key)).body(Body::empty()).unwrap()).await.unwrap();
    assert_eq!(task.status(), StatusCode::ACCEPTED);
    assert_eq!(response_json(task).await["task"]["status"], "completed");
    let persisted: Value = serde_json::from_slice(&fs::read(fixture.dir.join("video_jobs.json")).unwrap()).unwrap();
    assert_eq!(persisted["video-1"]["status"], "completed");

    let content = fixture.app.clone().oneshot(Request::get("/v1/videos/video-1/content").header("authorization", format!("Bearer {}", fixture.key)).body(Body::empty()).unwrap()).await.unwrap();
    assert_eq!(content.status(), StatusCode::ACCEPTED);
    assert_eq!(to_bytes(content.into_body(), usize::MAX).await.unwrap().as_ref(), b"\0\0\0\x18ftypmp42test");

    let denied = fixture.app.clone().oneshot(Request::get("/v1/videos/video-1").header("authorization", format!("Bearer {}", fixture.other_key)).body(Body::empty()).unwrap()).await.unwrap();
    assert_eq!(denied.status(), StatusCode::NOT_FOUND);
}

#[tokio::test]
async fn replay_returns_original_video_task_id_without_submitting_again() {
    let fixture = video_fixture();
    let submit = || Request::post("/v1/videos/generations")
        .header("authorization", format!("Bearer {}", fixture.key))
        .header("content-type", "application/json")
        .header("idempotency-key", "same-video-request")
        .body(Body::from(r#"{"model":"seedance","prompt":"测试重放"}"#))
        .unwrap();
    let first = fixture.app.clone().oneshot(submit()).await.unwrap();
    assert_eq!(first.status(), StatusCode::ACCEPTED);
    let replay = fixture.app.clone().oneshot(submit()).await.unwrap();
    assert_eq!(replay.status(), StatusCode::OK);
    assert_eq!(response_json(replay).await["task"]["id"], "video-1");
    assert_eq!(fixture.bridge.paths().iter().filter(|path| path.as_str() == "/v1/videos/generations").count(), 1);
}
