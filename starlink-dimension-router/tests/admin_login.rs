...
impl TestDir {
    fn new(label: &str) -> Self {
        Self(std::env::temp_dir().join(format!(
            "starlink-admin-login-test-{label}-{}",
            rand::random::<u64>()
        )))
    }
...
fn test_app_with_admin_password(password: &str) -> TestApp {
    let dir = TestDir::new("app");
    ...
    TestApp { app: build_router(state), _dir: dir }
}

async fn post_json(app: &TestApp, path: &str, value: Value) -> Response<Body> {
    app.app.clone().oneshot(Request::post(path).header("content-type", "application/json").body(Body::from(value.to_string())).unwrap()).await.unwrap()
}

async fn post_with_cookie(app: &TestApp, path: &str, cookie: &str, value: Value) -> Response<Body> {
    app.app.clone().oneshot(Request::post(path).header("content-type", "application/json").header("cookie", cookie).body(Body::from(value.to_string())).unwrap()).await.unwrap()
}

async fn patch_with_cookie(app: &TestApp, path: &str, cookie: &str, value: Value) -> Response<Body> {
    app.app.clone().oneshot(Request::patch(path).header("content-type", "application/json").header("cookie", cookie).body(Body::from(value.to_string())).unwrap()).await.unwrap()
}

async fn get(app: &TestApp, path: &str) -> Response<Body> {
    app.app.clone().oneshot(Request::get(path).body(Body::empty()).unwrap()).await.unwrap()
}

async fn get_with_cookie(app: &TestApp, path: &str, cookie: &str) -> Response<Body> {
    app.app.clone().oneshot(Request::get(path).header("cookie", cookie).body(Body::empty()).unwrap()).await.unwrap()
}
...
async fn admin_login_sets_cookie_and_session_reads_summary() {
    ...
    assert!(summary_json["core"]["active_api_keys"].is_null());
    assert_eq!(summary_json["credits"]["upstream_fresh"], false);
...
async fn admin_can_create_and_manage_key_without_precreating_user() {
    ...
    assert_eq!(quota.status(), StatusCode::GONE);
    assert_eq!(response_error_type(quota).await, "legacy_quota_write_disabled");
...
#[tokio::test]
async fn key_allocation_fails_closed_without_a_live_upstream_summary() {
    let app = test_app_with_admin_password("test-password");
    let login = post_json(&app, "/admin/v1/login", json!({"username":"admin","password":"test-password"})).await;
    let cookie = login.headers().get("set-cookie").unwrap().to_str().unwrap().to_owned();
    let created = post_with_cookie(&app, "/admin/v1/api-keys", &cookie, json!({
    ...
    assert_eq!(response_error_type(pool).await, "legacy_quota_write_disabled");
    ...
    assert_eq!(allocation.status(), StatusCode::SERVICE_UNAVAILABLE);
    assert_eq!(response_error_type(allocation).await, "upstream_credits_unavailable");
}
