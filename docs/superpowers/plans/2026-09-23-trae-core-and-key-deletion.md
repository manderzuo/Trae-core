...
- [ ] 本地验收和 Git 推送成功后，再部署到用户指定的公网 Core 服务器。先只读核实实际服务单元、部署目录、数据目录、反代入口和健康检查路径；部署前在服务器现有持久卷/备份目录做数据库与当前 release 备份，不把生产数据库或凭据拉回本机、不覆盖用户数据。发布 v21 代码后验证服务进程、健康检查及管理员 UI；异常时用服务器上的备份恢复旧 release，并报告结果。若无法唯一确认目标服务或无法建立可恢复备份，则停止服务器发布并向用户说明阻碍。

**验收：** 全量测试和 release 构建通过；仓库无秘密/运行数据；只发布到 Trae-core；服务器部署仅在核实目标并完成备份后执行；最终报告提交号、Git 远端状态、服务健康/回滚状态及仍存在的限制。
