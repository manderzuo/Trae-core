...
状态：Core-only 流式兼容与无头请求去重已完成模拟上游验收；真实公网生成仍受视频计费闸门约束
...
- 有 `Idempotency-Key` 时，按 Key + 规范化请求哈希稳定复用原任务；同 Key 不同内容 409。2026-09-24 用户确认其他电脑不改配置后，无该头的流式客户端改为由 Core 为单次请求生成并持久化内部编号，不再直接返回 400；同一 Key、规范化请求内容的运行中任务或两分钟内终结的任务会在数据库事务中复用。连接内不自动重发生成任务。跨窗口的自动重试与主动再次提交无法可靠区分，仍可能产生新任务和新费用；短时间主动重复相同内容则会复用旧任务，此模式不具备跨连接恰好一次保证。生产视频计费闸门继续独立控制，尚未核实真实回执前不得因此开放。
