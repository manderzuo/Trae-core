...
POST /v1/videos/generations（携带 asset_id）
      ↓
AI Work 按当前 Work 账号调用 Trae 原生上传地址
      ↓
TOS PUT 原始字节 + 提交结果，得到 store_uri
      ↓
Seedance 使用原生资源 URI 生成
```
...
- LAN、跨路由器和公网模式都不要求 Seedance 上游访问客户端电脑；素材只需能从调用方
  通过网关的 `/v1/assets` 上传到 AI Work 主机。
- `AIWORK_ASSET_PUBLIC_BASE_URL` 只在需要让客户端查看短时素材时配置，不参与上游
  输入图签名，也不需要把局域网地址暴露给 Trae。
...
- 只允许 HTTPS 生产访问；LAN 客户端入口测试可以先用 HTTP，但 API Key 仍必须启用。
  `AIWORK_ASSET_PUBLIC_BASE_URL` 若启用，仅用于客户端查看短时素材内容；默认强制 HTTPS，
  只有隔离测试显式设置 `AIWORK_ALLOW_INSECURE_ASSET_BASE=true` 才能使用 HTTP。
